...
unsigned char *gray(struct imgRawImage *image) {
    ...
    unsigned char *data = image->lpData;
    
    #pragma omp target teams distribute parallel for collapse(2) \
        map(to: data[0:3*width*height]) \
        map(from: output[0:3*width*height])
    for (int y = 0; y < height; ++y) {
        for (int x = 0; x < width; ++x) {
            unsigned char luma = (unsigned char) (
                    0.299f * (float) data[y*width*3+x*3+0]
                    + 0.587f * (float) data[y*width*3+x*3+1]
                    + 0.114f * (float) data[y*width*3+x*3+2]);
            output[y*width*3+x*3+0] = luma;
            output[y*width*3+x*3+1] = luma;
            output[y*width*3+x*3+2] = luma;
        }
    }
    return output;
}
