...
#pragma omp target teams distribute parallel for collapse(2)      \
    ...
    for (int y = 0; y < height; ++y)
    {
        for (int x = 0; x < width; ++x)
        {
            ...
            gx = -1 * pixels[(y_m1 * width + x_m1) * 3] + 1 * pixels[(y_m1 * width + x_p1) * 3] - 2 * pixels[(y * width + x_m1) * 3] + 2 * pixels[(y * width + x_p1) * 3] - 1 * pixels[(y_p1 * width + x_m1) * 3] + 1 * pixels[(y_p1 * width + x_p1) * 3];

            gy = -1 * pixels[(y_m1 * width + x_m1) * 3] - 2 * pixels[(y_m1 * width + x) * 3] - 1 * pixels[(y_m1 * width + x_p1) * 3] + 1 * pixels[(y_p1 * width + x_m1) * 3] + 2 * pixels[(y_p1 * width + x) * 3] + 1 * pixels[(y_p1 * width + x_p1) * 3];
