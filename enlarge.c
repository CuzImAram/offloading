...
unsigned int *calculateEnergySobel(struct imgRawImage *image) {
    ...
    for (int y = 0; y < height; ++y) {
        for (int x = 0; x < width; ++x) {
            ...
            for (int v = -4; v <= 4; ++v) {
                if (y + v < 0 || y + v >= height) continue;
                for (int u = -4; u <= 4; ++u) {
                    if (x + u < 0 || x + u >= width) continue;
...
int main(int argc, char *argv[]) {
    ...
    // NOTE: In a real scenario, you should free the original host memory of input->lpData
    // before overwriting the pointer to avoid a leak on the host side,
    // but we are focused on the offloading logic here.
