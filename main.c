...
#pragma omp target teams distribute parallel for collapse(2) \
    ...
    for (int y = 0; y < height; ++y)
    {
        for (int x = 0; x < width; ++x)
        {
            ...
            unsigned char luma = (unsigned char)(0.299f * (float)input[idx + 0] + 0.587f * (float)input[idx + 1] + 0.114f * (float)input[idx + 2]);
            output[idx + 0] = luma;
            output[idx + 1] = luma;
            output[idx + 2] = luma;
        }
...
#pragma omp target teams distribute parallel for map(to : data[0 : width * height]) \
...
#pragma omp target teams distribute parallel for collapse(2) \
    ...
    for (int y = 0; y < height; ++y)
    {
        for (int x = 0; x < width; ++x)
        {
            ...
            gx = -1 * input[(y_m1 * width + x_m1) * 3 + 0] + 1 * input[(y_m1 * width + x_p1) * 3 + 0] - 2 * input[(y * width + x_m1) * 3 + 0] + 2 * input[(y * width + x_p1) * 3 + 0] - 1 * input[(y_p1 * width + x_m1) * 3 + 0] + 1 * input[(y_p1 * width + x_p1) * 3 + 0];

            gy = -1 * input[(y_m1 * width + x_m1) * 3 + 0] - 2 * input[(y_m1 * width + x) * 3 + 0] - 1 * input[(y_m1 * width + x_p1) * 3 + 0] + 1 * input[(y_p1 * width + x_m1) * 3 + 0] + 2 * input[(y_p1 * width + x) * 3 + 0] + 1 * input[(y_p1 * width + x_p1) * 3 + 0];
...
#pragma omp target teams distribute parallel for map(to : minEnergySums[0 : width * height], seamStartIndices[0 : k]) \
...
#pragma omp target teams distribute parallel for map(to : oldData[0 : width * height * 3], seamPaths[0 : k * height]) \
