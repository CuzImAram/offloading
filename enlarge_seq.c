...
unsigned int *calculateEnergySobel(struct imgRawImage *image, int *entropy_lut)
{
    ...
    for (int y = 0; y < height; ++y)
    {
        for (int x = 0; x < width; ++x)
        {
            ...
            gx = -1 * image->lpData[(y_m1 * width + x_m1) * 3] + 1 * image->lpData[(y_m1 * width + x_p1) * 3] - 2 * image->lpData[(y * width + x_m1) * 3] + 2 * image->lpData[(y * width + x_p1) * 3] - 1 * image->lpData[(y_p1 * width + x_m1) * 3] + 1 * image->lpData[(y_p1 * width + x_p1) * 3];

            gy = -1 * image->lpData[(y_m1 * width + x_m1) * 3] - 2 * image->lpData[(y_m1 * width + x) * 3] - 1 * image->lpData[(y_m1 * width + x_p1) * 3] + 1 * image->lpData[(y_p1 * width + x_m1) * 3] + 2 * image->lpData[(y_p1 * width + x) * 3] + 1 * image->lpData[(y_p1 * width + x_p1) * 3];
